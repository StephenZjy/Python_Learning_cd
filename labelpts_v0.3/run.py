...
class PtsLabel(object):
    ...
    def load_last_pts(self):
        if self.last_id == -1:
            self.pts = []
        
        if abs(self.cur_id - self.last_id) > 1:
            self.pts = []

    def load_pts_from_file(self, pts_file):
        if not os.path.exists(pts_file):
            return False
        print('pts_fle:', pts_file)
        pts = read_points(pts_file)
        if len(pts) < self.max_pts_num:
            return False
        self.pts = pts
        return True
    ...
    def run(self):
        ...
        while is_exit == False:
            ...
            if self.first_read == True:
                is_load = self.load_pts_from_file(pts_file)
                if not is_load:
                    self.load_last_pts()
            ...
            if cur_id != self.cur_id:
                print(cur_id, len(self.images_files))
