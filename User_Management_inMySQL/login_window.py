...
class Ui_login(QtWidgets.QDialog):
    ...
    def retranslateUi(self, login):
        ...
        self.tel_label.setText(_translate("login", str(self.tel)))
        self.email_label.setText(_translate("login", str(self.email)))
        self.username_textEdit.setText(_translate("login", self.username))
        self.tel_textEdit.setText(_translate("login", str(self.tel)))
        self.email_textEdit.setText(_translate("login", str(self.email)))
    ...
    def get_user_info(self, _username):
        username, _, tel, email = self.mywindow.sqlOperation.select_username(_username)
        return username, tel, email

    def load_info(self):
        self.mywindow.sqlOperation.cursor.execute('SELECT username, tel, email FROM user_info')
        info = self.mywindow.sqlOperation.cursor.fetchall()

        for row, data in enumerate(info):
            username = data[0]
    ...
    def on_tableWidget_Clicked(self, item):
        ...
        self.click_x = cursor_pos.x()
        self.click_y = cursor_pos.y()
        # 调整 x 和 y 以适应您的需求，例如在右侧
        self.click_x = self.click_x - self.width() + 20
    ...
    def show_user_window(self):
        ...
        user_window.set_position(self.click_x, self.click_y)
        user_window.show()
        user_window.exec_()
...
class Ui_user_window(QtWidgets.QDialog):
    ...
    def retranslateUi(self, user_window):
        _translate = QtCore.QCoreApplication.translate
        username, tel, email = self.login_instance.get_user_info(self.login_instance.current_username)
